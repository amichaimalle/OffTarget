...
//#define MASK_TO_CHAR(mask) (mask&0X88 ? 'A' : mask&0X44 ? 'C' : mask&0X22 ? 'G' : 'T')
//#define CHAR_TO_CODE(char) {char&0x80 ? 0 : char&0x40 ? 1 : char&0x20 ? 2 : 3 , char&0x08 ? 0 : char&0x04 ? 1 : char&0x02 ? 2 : 3}
...
void DecodeAlignment(Guide *GuideInfo, ChromosomeInfo *Chromosome, OffTarget *offTarget, char ReverseMask);
// Post Processing
...
int ReadChromosome(ChromosomeInfo *Chromosome) {
    ...
    while (fscanf(ChrFile, "%[^\n]c", lineBuffer) != EOF) { //read until new line
        ...
        for (int i=0;i<MAX_LINE_SIZE;i++){ // while(lineBuffer[i] != '\0' && lineBuffer[i] != '\n'){
    ...
    for (int i = 0; i < Chromosome->Length; i++) {
        ...
        } else if (Chromosome->Text[Chromosome->TextInx--] & 0x20) {
            printf("G");
    ...
    Chromosome->TextInx = Chromosome->Length;
    fclose(ChrFile);
    return 0;
}
...
void OffFinderMainLoop(Guide **guideLst, ChromosomeInfo *Chromosome, Pam *PamInfo, OffTarget **OffTargetHead){
    ...
    //int ValidPam = 1, ValidReversePam = 1; // default is  if pam is null
    int ValidPam[2] = {1, 1}; // default is  if pam is null
    int Reverse;
    //printf("test0\n");
    ...
    while (Chromosome->TextInx > 0) { // run from tail to head of chromosome
        ...
        NucleotideCode[0] = Nucleotide&0x08 ? 0 : Nucleotide&0x04 ? 1 : Nucleotide&0x02 ? 2 : 3;
        NucleotideCode[1] = Nucleotide&0x80 ? 0 : Nucleotide&0x40 ? 1 : Nucleotide&0x20 ? 2 : 3;
        //printf("test01\n");
        if (max_pam_mismatch != -1) {
            PamNucleotide = Chromosome->Text[Chromosome->TextInx - PamInfo->Length];
            PamNucleotideCode[0] = PamNucleotide&0x08 ? 0 : PamNucleotide&0x04 ? 1 : PamNucleotide&0x02 ? 2 : 3;
            PamNucleotideCode[1] = PamNucleotide&0x80 ? 0 : PamNucleotide&0x40 ? 1 : PamNucleotide&0x20 ? 2 : 3;
        ...
        for (int GuideNum = 0; GuideNum < NumOfGuides; GuideNum++) { // run on all reverse guides
            Reverse = GuideNum%2;
            BitMaskVector = guideLst[GuideNum]->BitMaskVectors[NucleotideCode[Reverse]];
            ...
            if (ValidPam[Reverse]) {
                if (CheckForAlignment(guideLst[GuideNum], tempOffTarget, Chromosome->TextInx) == 0) {
                    //printf("test20\n");
                    AddOffTargetToList(guideLst[GuideNum], Chromosome, &PamInfo[Reverse], OffTargetHead, tempOffTarget);
...
void AddOffTargetToList(Guide *GuideInfo, ChromosomeInfo *Chromosome, Pam *PamInfo, OffTarget **OffTargetHead, OffTarget *offTargetToAdd){
    char ReverseMask = (GuideInfo->Strand == '+') ? 0xf0 : 0x0f;
    ...
    DecodeAlignment(GuideInfo, Chromosome, offTargetToAdd, ReverseMask);
    if (max_pam_mismatch != -1) {
...
void DecodeAlignment(Guide *GuideInfo, ChromosomeInfo *Chromosome, OffTarget *offTarget, char ReverseMask){
    ...
    int GuideInx = 0;
    int SiteInx = Chromosome->TextInx - Chromosome->Length - 1;
    ...
    for (int i=0; i<AlignmentLength; i++) {
        switch (GuideInfo->EncodeAlignment[i]){
            case 1: // Match
                ...
                SiteAlignment[i] = MASK_TO_CHAR(Chromosome->Text[SiteInx]&&ReverseMask);
                SiteInx++;
                break;
            case 2: // Mismatch
                GuideAlignment[i] = GuideInfo->Read[GuideInx++];
                SiteAlignment[i] = (char)tolower(MASK_TO_CHAR(Chromosome->Text[SiteInx]&&ReverseMask));// + 32;
                SiteInx++;
                break;
            case 3: // Deletion
                GuideAlignment[i] = GuideInfo->Read[GuideInx++];
    ...
    offTarget->ChromosomePosition = (GuideInfo->Strand == '+') ? Chromosome->TextInx - Chromosome->Length -AlignmentLength : Chromosome->TextInx;
